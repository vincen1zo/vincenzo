<%@page import="vo.MemberEditVo"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	MemberEditVo vo = (MemberEditVo)request.getAttribute("mVo");
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<div class="user_info">
		<div class="user_info_title">
			<div>회원정보수정</div><a href="#" class="jy_outSite">탈퇴하기</a>
		</div>
		<form action="Controller?command=Mypage_setting_edit_user_info" method="post">
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						이메일<div class="user_insert_title_small">* 필수항목</div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
						<div class="item_input">
							<span class="input_section">
								<label>
									<input class="form_control form_size jy_new_size" name="email" value="<%=vo.getEmail() %>" type="text" placeholder="이메일" readonly/><!-- 이메일 넣기 -->
								</label>
							</span>
<!-- 							<span class="rhfqoddl">@</span>
							<span>
								<label>
									<select class="form_control form_size">
						...
						<div class="dnsdudwk">이메일을 변경하시려면 운영자에게 이메일을 보내주세요.</div>
					</div>
				</div>
			</div>
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						별명<div class="user_insert_title_small">* 필수항목</div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
						<div class="item_input">
							<span class="input_section">
								<label>
									<input class="form_control form_size2" name="member_id" value="<%=vo.getMemberId()%>" type="text" placeholder="별명을 지어봐용~~" required/><!-- 별명 넣기 -->
								</label>
							</span>
						</div>
			...
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						홈페이지<div class="user_insert_title_small"></div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
						<div class="item_input">
							<span class="input_section">
								<label>
									...
									<input class="form_control form_size2" name="homepage" value="" type="text" placeholder="https://ohou.se"/>
									<%} else { %>
									<input class="form_control form_size2" name="homepage" value="<%=vo.getHomepage()%>" type="text" placeholder="https://ohou.se"/><!-- 별명 넣기 -->
									<%} %>
								</label>
							</span>
						</div>
			...
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						성별<div class="user_insert_title_small"></div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
					<%if(vo.getGender()==1) {%>
						<div class="item_input">
							<label>
								<input class=" form_control gen" type="radio" name="gender" value="1" checked/><!-- 성별 체크 -->
							</label>
							<span class="gen_der">남성</span>	
							<label>
								<input class=" form_control gen" type="radio" name="gender" value="2"/>
							</label>
							<span class="gen_der">여성</span>
						</div>
					<%} else if((vo.getGender()==2)) { %>
						<div class="item_input">
							<label>
								<input class=" form_control gen" type="radio" name="gender" value="1" /><!-- 성별 체크 -->
							</label>
							<span class="gen_der">남성</span>	
							<label>
								<input class=" form_control gen" type="radio" name="gender" value="2" checked/>
							</label>
							<span class="gen_der">여성</span>
						</div>
					<%} else {%>
			...
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						생년월일<div class="user_insert_title_small"></div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
						<div class="item_input">
							<span class="input_section">
								<label>
									...
									<input class="form_control form_size2" name="birthday" value="" type="text" placeholder="YYYY-MM-DD"/><!-- 날짜 넣기 -->
									<%} else { %>
									<input class="form_control form_size2" name="birthday" value="<%=vo.getBirthday() %>" type="text" placeholder="YYYY-MM-DD"/><!-- 날짜 넣기 -->
									<%} %>
								</label>
							</span>
						</div>
			...
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						프로필 이미지<div class="user_insert_title_small"></div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
						<div class="img_tl">
							<button class="insert_img_btn" type="button">
								<%
									if(vo.getProfileImg()==null) {
								%>
								<img class="profile_img" name="profile_img" src="images/profile_img.png"/>
								<%
									} else {
								%>
								<img class="profile_img" src="<%=vo.getProfileImg()%>"/>
								<%}%>
							</button>
						</div>
					</div>
			...
			<div class="user_insert_area"><!-- 시작점 -->
				<div class="user_insert_title">
					<div>
						한줄 소개<div class="user_insert_title_small"></div>
					</div>
				</div>
				<div class="user_input input_area">
					<div>
						<div class="item_input">
							<span class="input_section">
								<label>
									<%if(vo.getOneLiner()==null) {%>
									<input class="form_control form_size2" name="one_liner" value="" type="text" placeholder="한줄 소개!~" maxlength="15"/>
									<%} else { %>
									<input class="form_control form_size2" name="one_liner" value="<%=vo.getOneLiner() %>" type="text" placeholder="한줄 소개!~" maxlength="15"/><!-- 별명 넣기 -->
									<%} %>
								</label>
							</span>
						</div>
			...
			<button class="user_info_submit_btn">회원 정보 수정</button>
		</form>
	</div>
	<!-- 회원정보수정 끝 -->
	<footer id="footer">
